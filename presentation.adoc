...
У проекта _нет_ другого места для хранения "первичных" данных
...
В каждой ветке свой файл пайплайна => можно безопасно экспериментировать с процессом сборки
...
Jenkins не умеет в приватные сообщения icon:sad-tear[]
